/* 
 * 
 * Базируется на оригинальном парсере parse.y руби версии  1.9.2-p136, разработанным Yukihiro Matsumoto.
 * Лексер реализован в parser_yylex, вызов которой осуществляется внутри yylex
 * 
 *
...
command: operation command_args             %prec tLOWEST
    ...
    | tSUPER call_args  { $$ = alloc_node(token_super, $2, NULL); }
...
string_content: tSTRING_CONTENT { $$ = alloc_node(token_bare_string, NULL, NULL); $$->value = strdup(parser->string_buf); printf("Ww\n"); printf("%s", parser->string_buf); memset(parser->string_buf, 0, sizeof(parser->string_buf));}
    | tSTRING_DBEG
    {
        lex_state = EXPR_BEG;
...
simple_numeric: tINTEGER        { $$ = alloc_node(token_numeric, NULL, NULL); $$->flags = int_l; $$->value = strdup(parser->string_val);}
    | tFLOAT                    { $$ = alloc_node(token_numeric, NULL, NULL); $$->flags = float_l; $$->value = strdup(parser->string_val);}
    | tRATIONAL
    {
		...
		$$->value = strdup(parser->string_val);
    }
    | tIMAGINARY
    {
        if (parser->version < ruby21) {
